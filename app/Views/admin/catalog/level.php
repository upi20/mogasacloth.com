					...
					<div>
						...
						<div class="widget-body">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								<thead>			                
									<tr>
										<th data-hide="phone"><i class="fa fa-fw fa-code text-muted hidden-md hidden-sm hidden-xs"></i> Kode</th>
										<th data-hide="phone"><i class="fa fa-fw fa-code text-muted hidden-md hidden-sm hidden-xs"></i> Level</th>
								...
								<tbody>
									...
									<tr data-id="<?= $q['vari_id'] ?>">
										<td><?=$q['vari_id']?></td>
										<td><?=$q['vari_level']?></td>
										<td><?=$q['vari_status']?></td>
										<td>
											<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['vari_id'] ?>, '<?= $q['vari_level'] ?>', '<?= $q['vari_status'] ?>')">
												<i class="fa fa-edit"></i> Update
											</button>
											<button class="btn btn-danger btn-sm" onclick="Delete(<?= $q['vari_id'] ?>)">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/catalog-level-create',
	    			data: {
	    				vari_level: level,
	    				vari_status: status,
	    			}
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/catalog-level-update',
	    			data: {
	    				vari_id: id,
	    				vari_level: level,
	    				vari_status: status,
    	...
    	{ 
	    	...
	    	$.ajax({
    			method: 'delete',
    			url: '<?= base_url ?>admin/catalog-level-delete',
    			data: {
    				color_id: id
    			}
    		})
