			...
			else
			{
				...
				if($check->num_rows < 1)
				{
					...
					$create_cart_detail = Cart_detail::create([
						...
						'card_price' => diskon($product['prod_price'], $product['prod_diskon']),
						'card_qty' => $data['cart_qty'],
						'card_total_price' => (int)diskon($product['prod_price'], $product['prod_diskon']) * (int)$data['cart_qty'],
						'card_status' => 1,
					]);
						...
						if($cd['card_prod_id'] === $data['cart_prod_id'])
						{
							...
							if($cd['cart_size_id'] === $data['cart_size_id'] AND $cd['cart_color_id'] === $data['cart_color_id'])
							{
								...
								$update_cart_detail = Cart_detail::update(['card_id'=> $cd['card_id']], [
									'card_qty' => $cart_qty,
									'card_total_price' => (int)$cart_qty * (int)diskon($product['prod_price'], $product['prod_diskon']),
								]);
